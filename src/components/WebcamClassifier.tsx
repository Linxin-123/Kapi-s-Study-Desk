...
interface WebcamClassifierProps {
  ...
  onSensingChange?: (active: boolean) => void;
}
...
export default function WebcamClassifier({
  ...
  const initLockRef = useRef<boolean>(false); // prevents concurrent/double model loads (React StrictMode)
  const loadedUrlRef = useRef<string | null>(null); // remembers which model URL is currently loaded
  ...
  // Tell the app whether real sensing is happening — timers only run while true.
  useEffect(() => {
    const active = isSimulatorMode || (cameraActive && modelLoaded && !error);
    onSensingChange?.(active);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isSimulatorMode, cameraActive, modelLoaded, error]);

  const [predictions, setPredictions] = useState<Array<{ className: string; probability: number }>>([]);

  // Load Teachable Machine dependencies from CDN — ONCE per model type.
  // Do NOT remove/reload scripts on every init: reloading tfjs while a model
  // is registered causes "Variable Conv1/kernel is already registered" errors.
  const loadDependencies = (type: 'image' | 'pose'): Promise<void> => {
    return new Promise((resolve, reject) => {
      // Already loaded with the right library? Reuse it.
      if (type === 'image' && window.tf && window.tmImage) { resolve(); return; }
      if (type === 'pose' && window.tf && window.tmPose) { resolve(); return; }

      // Only clear when the WRONG library version is present (type switch)
      const wrongLibPresent =
        (type === 'image' && window.tmPose) || (type === 'pose' && window.tmImage);
      if (wrongLibPresent) {
        document.querySelectorAll('script[data-tm-script]').forEach((el) => el.remove());
        try { delete (window as any).tf; } catch (_) {}
        try { delete (window as any).tmImage; } catch (_) {}
        try { delete (window as any).tmPose; } catch (_) {}
      }
      ...
      const loadScript = (src: string): Promise<void> =>
        new Promise((res, rej) => {
          ...
          s.src = src;
          s.async = false; // sequential loading to avoid race conditions
          s.setAttribute('data-tm-script', 'true');
          s.onload = () => res();
  ...
  const initModel = async () => {
    ...
    // (Also blocks React 18 StrictMode double-effect from loading twice,
    //  which causes "Variable Conv1/kernel is already registered".)
    if (loadedUrlRef.current === formattedUrl && modelRef.current) {
      setModelLoaded(true);
      return;
    }
    if (initLockRef.current) return; // a load is already in flight
    ...
    try {
      ...
      // 2) Load the matching library versions (cached after first load)
      await loadDependencies(detectedType);

      // 3) Dispose the PREVIOUS model first so its weight variables are
      //    unregistered — prevents "already registered" collisions.
      if (modelRef.current) {
      ...
      // 4) Load the model — with ONE silent retry (CDN/network hiccups)
      const doLoad = () =>
        detectedType === 'pose'
          ? window.tmPose.load(modelJson, metadataJson)
          : window.tmImage.load(modelJson, metadataJson);

      let loadedModel;
      try {
        loadedModel = await doLoad();
      } catch (firstErr: any) {
        console.warn('First model load failed, retrying once...', firstErr);
        await new Promise((r) => setTimeout(r, 800));
        loadedModel = await doLoad();
      }

      modelRef.current = loadedModel;
      loadedUrlRef.current = formattedUrl;
      setModelLoaded(true);
      setLoading(false);
    } catch (err: any) {
      console.error(err);
    ...
    const predictLoop = async () => {
      ...
      if (modelRef.current && cameraActive && videoRef.current && videoRef.current.readyState === 4) {
        try {
          ...
          let maxProb = 0;
          let bestClass: FocusState = 'absent'; // default to absent if nothing is confident

          prediction.forEach((p: any) => {
            if (p.probability > maxProb) {
              maxProb = p.probability;
              // Map prediction labels safely to FocusState keys
              // New model has 3 classes: Focus / Distracted / Absent
              const label = p.className.toLowerCase().trim();
              if (label.includes('focus')) bestClass = 'focused';
              else if (label.includes('distra') || label.includes('phone') || label.includes('mobile') || label.includes('tired') || label.includes('sleep')) bestClass = 'distracted';
              else if (label.includes('absent') || label.includes('away') || label.includes('none') || label.includes('empty')) bestClass = 'absent';
            ...
            <div className="bg-[#FFFDFB] rounded-2xl border border-[#FFEADA] p-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  { key: 'distracted', label: 'Distracted 😵', color: 'bg-amber-50 text-amber-800 border-amber-200 hover:bg-amber-100' },
                  { key: 'absent', label: 'Absent 🚪', color: 'bg-blue-50 text-blue-800 border-blue-200 hover:bg-blue-100' },
                ].map((btn) => (
                  <button
                    key={btn.key}
                    onClick={() => onStateDetected(btn.key as FocusState, true)}
          ...
          {!isSimulatorMode && !modelUrl && (
            <div className="bg-[#FFF9F2] p-3.5 rounded-2xl border border-[#FFE4CF] text-[11px] text-[#725442] leading-relaxed">
              ...
              <p className="text-[#A0836D]">
                Train your 3-class model (Focus / Distracted / Absent) on{' '}
                <a
