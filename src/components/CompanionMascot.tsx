...
const stateMotion: Record<FocusState, { animate: any; transition: any }> = {
  ...
  distracted: { animate: { y: [0, -4, 0], x: 8 }, transition: { duration: 1.4, repeat: Infinity, ease: 'easeInOut' } },
  absent: { animate: { y: [3, 7, 3] }, transition: { duration: 4.6, repeat: Infinity, ease: 'easeInOut' } },
...
const CAPTIONS: Record<FocusState, (name: string) => string> = {
  ...
  distracted: () => `Psst… let's refocus! You've got this 💪`,
  absent: (n) => `${n} and Luna are napping while they wait for you`,
  stretch: () => `Great job!! You're doing amazing`,
};
...
function KapiMouth({ state }: { state: FocusState }) {
  ...
  if (state === 'distracted') return <ellipse cx="167" cy="211" rx="4.5" ry="5.5" fill={INK} />;
  // gentle smile (focused / absent / stretch)
...
function Kapi({ state }: { state: FocusState }) {
  return (
    <g>
      ...
      <motion.g
        animate={{ rotate: state === 'distracted' ? 6 : 0 }}
      ...
      {/* ---- Distracted: holding phone to the chest ---- */}
      {state === 'distracted' && (
        <g>
          ...
          >
            <rect x="186" y="222" width="18" height="24" rx="3" fill="#444" />
            <rect x="188" y="224" width="14" height="18" rx="2" fill="#88C0D0" opacity="0.8" />
            <circle cx="195" cy="244" r="1.5" fill="#888" />
          </motion.g>
        </g>
      )}
    </g>
...
function Luna({ state }: { state: FocusState }) {
  // Luna cuddles closer when Kapi is distracted (to comfort)
  const dx = state === 'distracted' ? -16 : 0;
  return (
...
export default function CompanionMascot({
  ...
  return (
    <div
      className={`relative flex flex-col items-center justify-end rounded-3xl border border-[#FFDEC9] shadow-md w-full mx-auto overflow-hidden ${
        compact ? 'h-[300px] p-4 pb-3' : 'h-[520px] p-6 pb-5'
      }`}
    ...
    >
      ...
      <div className="relative w-full flex-1 flex items-end justify-center min-h-0 z-10">
        <motion.svg
          ...
          className={`w-full h-auto ${compact ? 'max-w-[17rem]' : 'max-w-[26rem]'}`}
          animate={m.animate}
