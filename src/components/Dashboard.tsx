...
export default function Dashboard({
  ...
  const getTodayTimelineData = () => {
    ...
    if (!stats.snapshots || stats.snapshots.length === 0) {
      return [
        { hour: '09:00', Focus: 45, Phone: 5, Away: 10 },
        { hour: '10:00', Focus: 50, Phone: 2, Away: 8 },
        { hour: '11:00', Focus: 30, Phone: 15, Away: 15 },
        { hour: '14:00', Focus: 55, Phone: 0, Away: 5 },
        { hour: '15:00', Focus: 40, Phone: 10, Away: 10 },
        { hour: '16:00', Focus: 48, Phone: 4, Away: 8 },
      ];
    }
    ...
    stats.snapshots.forEach((snap) => {
      ...
      if (!hourlyBins[hourStr]) {
        hourlyBins[hourStr] = { focused: 0, phone: 0, away: 0, tired: 0, stretch: 0 };
      }
      // Add weight (assume snapshots represent 5-second intervals during logging)
      hourlyBins[hourStr][snap.state] += 5;
    });
    ...
    return hours.map((h) => {
      ...
      return {
        ...
        Phone: Math.round(bin.phone / 60),
        Away: Math.round((bin.away + bin.tired) / 60),
  ...
  const getWeeklyTrendData = () => {
    // If weekly stats is empty, provide beautiful placeholder
    if (!weeklyStats || weeklyStats.length <= 1) {
      return [
        { name: 'Mon', FocusTime: 120, Pomodoros: 4, Distractions: 3 },
        { name: 'Tue', FocusTime: 150, Pomodoros: 5, Distractions: 2 },
        { name: 'Wed', FocusTime: 90, Pomodoros: 3, Distractions: 5 },
        { name: 'Thu', FocusTime: 180, Pomodoros: 6, Distractions: 1 },
        { name: 'Fri', FocusTime: 210, Pomodoros: 7, Distractions: 0 },
        { name: 'Sat', FocusTime: 140, Pomodoros: 4, Distractions: 4 },
        { name: 'Sun', FocusTime: 240, Pomodoros: 8, Distractions: 1 },
      ];
    }
    ...
    return weeklyStats.map((ws) => {
      ...
      return {
        name: dayName,
        FocusTime: Math.round(ws.totalFocusTime / 60),
        Pomodoros: ws.pomodorosCompleted,
        Distractions: ws.distractionCount,
      };
    });
  };
  ...
  // Active statistics
  const currentTotalFocus = stats.totalFocusTime || 0;
  const currentPhoneCount = stats.distractionCount || 0;
  const currentPomos = stats.pomodorosCompleted || 0;
  const currentStreak = stats.longestStreakMinutes || 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-[#FFDEC9] rounded-3xl p-5 flex flex-col justify-between shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-[#A0836D]">Phone Distractions</span>
          ...
          <div className="mt-4">
            ...
            <span className="text-[9px] text-[#C68A4C] font-bold bg-amber-50 px-2 py-0.5 rounded-md mt-1.5 inline-block">
              {currentPhoneCount > 5 ? 'A bit restless today' : 'Excellent phone control'}
            </span>
          </div>
        </div>
      ...
      {/* Orange Hot Spring — every completed Pomodoro drops one orange into Kapi's onsen */}
      <div className="bg-white border border-[#FFDEC9] rounded-3xl p-6 shadow-xs relative overflow-hidden">
        <div className="flex items-center gap-2 mb-1">
          <span className="text-lg">🍊</span>
          <h3 className="font-extrabold text-[#725442] text-sm">Kapi's Orange Hot Spring</h3>
        </div>
        <p className="text-[11px] text-[#A0836D] mb-4">
          Every Pomodoro you finish drops one warm orange into the onsen. Today's harvest:{' '}
          <b className="text-[#FF9E7D]">{currentPomos} {currentPomos === 1 ? 'orange' : 'oranges'}</b>
          {currentPomos >= 6 && ' — the spring is getting full! Amazing! ✨'}
        </p>

        <svg viewBox="0 0 380 150" className="w-full max-w-md mx-auto block" role="img" aria-label={`Hot spring with ${currentPomos} oranges`}>
          {/* Steam wisps */}
          <path d="M120 42 C116 32 124 28 120 18" stroke="#E8D5C4" strokeWidth="4" strokeLinecap="round" fill="none" opacity="0.7" />
          <path d="M190 36 C186 26 194 22 190 12" stroke="#E8D5C4" strokeWidth="4" strokeLinecap="round" fill="none" opacity="0.7" />
          <path d="M260 42 C256 32 264 28 260 18" stroke="#E8D5C4" strokeWidth="4" strokeLinecap="round" fill="none" opacity="0.7" />

          {/* Rock rim */}
          <ellipse cx="190" cy="100" rx="150" ry="42" fill="#D8C3AC" />
          <ellipse cx="52" cy="94" rx="20" ry="14" fill="#C9B098" />
          <ellipse cx="330" cy="96" rx="18" ry="13" fill="#C9B098" />
          <ellipse cx="120" cy="132" rx="22" ry="12" fill="#C9B098" />
          <ellipse cx="268" cy="134" rx="20" ry="11" fill="#C9B098" />

          {/* Water */}
          <ellipse cx="190" cy="96" rx="128" ry="32" fill="#BFE3E0" />
          <ellipse cx="190" cy="94" rx="128" ry="30" fill="#D2EEEB" />
          <path d="M100 92 Q120 88 140 92 T180 92" stroke="#B2DCD8" strokeWidth="2.5" strokeLinecap="round" fill="none" />
          <path d="M210 102 Q230 98 250 102 T290 102" stroke="#B2DCD8" strokeWidth="2.5" strokeLinecap="round" fill="none" />

          {/* Floating oranges: one per completed pomodoro (display caps at 8) */}
          {Array.from({ length: Math.min(currentPomos, 8) }).map((_, i) => {
            const positions = [
              { x: 150, y: 90 }, { x: 205, y: 98 }, { x: 245, y: 88 }, { x: 118, y: 100 },
              { x: 178, y: 82 }, { x: 270, y: 100 }, { x: 95, y: 88 }, { x: 228, y: 108 },
            ];
            const p = positions[i];
            return (
              <g key={i}>
                <ellipse cx={p.x} cy={p.y + 11} rx="12" ry="3.5" fill="#A8CFCB" opacity="0.6" />
                <circle cx={p.x} cy={p.y} r="12" fill="#F5A04A" />
                <circle cx={p.x - 4} cy={p.y - 4} r="2.8" fill="#FDBE77" />
                <path d={`M${p.x + 2} ${p.y - 11} C${p.x + 6} ${p.y - 14} ${p.x + 9} ${p.y - 11} ${p.x + 7} ${p.y - 9} C${p.x + 4} ${p.y - 7} ${p.x + 1} ${p.y - 9} ${p.x + 2} ${p.y - 11} Z`} fill="#8FBC6F" />
              </g>
            );
          })}

          {/* Overflow badge when more than 8 pomodoros */}
          {currentPomos > 8 && (
            <g>
              <circle cx="322" cy="70" r="16" fill="#FF9E7D" />
              <text x="322" y="75" textAnchor="middle" fill="white" fontSize="12" fontWeight="bold">
                +{currentPomos - 8}
              </text>
            </g>
          )}

          {/* Empty-spring encouragement */}
          {currentPomos === 0 && (
            <text x="190" y="100" textAnchor="middle" fill="#8FB5B1" fontSize="12" fontWeight="bold">
              Finish a Pomodoro to drop in your first orange!
            </text>
          )}
        </svg>
      </div>
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#A0836D] mt-0.5">
              {activeTab === 'today' 
                ? 'Detailed hourly breakdown of your focus ratios, phone breaks, and resting intervals.'
                : 'Comparative view of your daily focus duration vs. phone usage for the past 7 days.'
              }
            </p>
          </div>
        </div>
          ...
          {activeTab === 'today' ? (
            <>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span>Phone Break / Distraction</span>
              </div>
              <div className="flex items-center gap-1.5">
