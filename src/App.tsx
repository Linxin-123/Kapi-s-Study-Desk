  ...
  useEffect(() => {
    ...
    if (cachedSettings) {
      try {
        ...
        // MIGRATION: old cached settings may point to the previous model —
        // force-upgrade to the new default model URL if an old one is stored
        if (
          typeof parsed.modelUrl === 'string' &&
          parsed.modelUrl.includes('UwvrV24Ma')
        ) {
    ...
    if (sessionState === 'running' && sensingActive) {
      timerRef.current = setInterval(() => {
        ...
        if (breakMode) {
          ...
          if (stableState === 'distracted') updated.totalDistractedTime += 1;
          else if (stableState === 'absent') updated.totalAbsentTime += 1;
          else if (stableState === 'stretch') updated.totalStretchTime += 1;

          if (stableState === 'absent' || stableState === 'stretch') {
          ...
          if (settings.healthEnabled !== false) {
            ...
            } else if (stableState === 'absent' || stableState === 'stretch') {
      ...
      <header className="sticky top-0 z-40 bg-white/70 backdrop-blur-md border-b border-[#FFE4CF]">
        <div className="max-w-3xl mx-auto px-5 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-sm font-extrabold tracking-tight">{settings.mascotName}'s Study Buddy</h1>
          </div>
            ...
            >
              ...
              <div className="mt-5 max-w-sm mx-auto">
                ...
                <p className="text-[10px] font-bold text-[#A0836D] mt-1.5">
                  {stableState === 'absent' || stableState === 'stretch'
                    ? '🌱 resting detected — the bar is growing!'
                    : '👀 still at the desk... step away or stretch and the bar will move'}
                </p>
              </div>
