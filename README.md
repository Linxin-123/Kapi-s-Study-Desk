...
This app has both a local Express server (`server.ts`, used by `npm run dev`) and Vercel serverless functions (`api/` folder, used automatically when deployed on Vercel). No code changes needed.

1. Push this folder to a GitHub repository — do **NOT** include `node_modules` or `.env.local` (both are already in `.gitignore`).
2. On [vercel.com](https://vercel.com): **Add New → Project → Import** your GitHub repo. Vercel auto-detects Vite via `vercel.json`.
3. Before clicking Deploy, open **Environment Variables** and add: name `GEMINI_API_KEY`, value = your Gemini API key.
4. Deploy. The chat and AI speech work through `/api/gemini/*` serverless functions.
