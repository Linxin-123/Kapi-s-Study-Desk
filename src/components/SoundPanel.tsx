...
export default function SoundPanel() {
  ...
  // The break screen's "Play Music" button opens this panel via a window event
  useEffect(() => {
    const openHandler = () => setOpen(true);
    window.addEventListener('kapi-open-sound-panel', openHandler);
    return () => window.removeEventListener('kapi-open-sound-panel', openHandler);
  }, []);
  ...
  return (
    <>
      <button
        ...
        className={`fixed bottom-5 right-[5.5rem] z-[70] w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-105 cursor-pointer ${
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-[5.5rem] z-[70] bg-[#FFFDF9] border border-[#FFDEC9] rounded-3xl shadow-2xl p-3.5 w-64 max-h-[75vh] overflow-y-auto"
          >
