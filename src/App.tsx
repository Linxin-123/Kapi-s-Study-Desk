 ...
 *
 * Kapi Companion — minimal, warm study buddy.
...
const DEFAULT_SETTINGS: UserSettings = {
  mascotName: 'Kapi',
  userName: '',
  modelUrl: 'https://teachablemachine.withgoogle.com/models/xNm9nH9aRW/',
  speechEnabled: false, // TTS voice OFF by default — enable in Settings
  chimeEnabled: false, // alert chimes OFF by default — enable in Settings
...
// ============================================================
// SOFT MUSIC-BOX CHIMES (v19.6)
// The old synth used a bare fifth (1.5×) partial with no filter,
// which sounded metallic/robotic. This engine sounds warm because:
//   • pure sine + a QUIET octave partial (slightly detuned = organic)
//   • a low-pass filter @2.6kHz removes the sharp digital edge
//   • soft 60ms attack (no click) + long 2–3s natural ring-out
//   • gentle overall volume
// ============================================================
let softCtx: AudioContext | null = null;
let softMaster: GainNode | null = null;

const ensureSoftAudio = (): AudioContext | null => {
  ...
  try {
    if (!softCtx) {
      ...
      if (!AC) return null;
      softCtx = new AC();
      softMaster = softCtx.createGain();
      softMaster.gain.value = 0.35; // gentle overall loudness
      const softener = softCtx.createBiquadFilter();
      softener.type = 'lowpass';
      softener.frequency.value = 2600;
      softener.Q.value = 0.5;
      softMaster.connect(softener);
      softener.connect(softCtx.destination);
    }
    if (softCtx.state === 'suspended') softCtx.resume();
    return softCtx;
  } catch (err) {
...
const softNote = (freq: number, when: number, dur = 2.0, vol = 1.0) => {
  const ctx = softCtx;
  if (!ctx || !softMaster) return;
  const t0 = ctx.currentTime + when;

  const gain = ctx.createGain();
  gain.gain.setValueAtTime(0.0001, t0);
  gain.gain.exponentialRampToValueAtTime(vol, t0 + 0.06); // soft attack
  gain.gain.exponentialRampToValueAtTime(0.0001, t0 + dur); // long ring-out
  gain.connect(softMaster);
  ...
  g2.gain.value = 0.28; // quiet octave partial → warm "wooden" character
  const o2 = ctx.createOscillator();
  o2.type = 'sine';
  o2.frequency.value = freq * 2;
  o2.detune.value = 4; // tiny detune = organic shimmer, less "machine"
  o2.connect(g2);
  g2.connect(gain);
...
type ChimeKind = 'breakStart' | 'breakEnd' | 'orange' | 'sessionStart';

const playSoftChime = (kind: ChimeKind) => {
  if (!ensureSoftAudio()) return;
  try {
    switch (kind) {
      case 'breakStart':
        // Slow descending phrase (G5 → E5 → C5): "wind down, time to rest"
        softNote(783.99, 0.0, 2.2, 0.9);
        softNote(659.25, 0.45, 2.2, 0.85);
        softNote(523.25, 0.9, 2.8, 0.8);
        break;
      case 'breakEnd':
        // Gentle rising phrase (C5 → E5 → G5): "recharged, back we go"
        softNote(523.25, 0.0, 1.8, 0.75);
        softNote(659.25, 0.35, 1.8, 0.75);
        softNote(783.99, 0.7, 2.4, 0.8);
        break;
      case 'orange':
        // Tiny bright sparkle for a harvested orange 🍊
        softNote(1046.5, 0.0, 1.2, 0.55);
        softNote(1318.5, 0.12, 1.4, 0.45);
        break;
      case 'sessionStart':
        // Soft "let's begin" (C5 → G5)
        softNote(523.25, 0.0, 1.6, 0.7);
        softNote(783.99, 0.3, 2.0, 0.7);
        break;
    }
  } catch (err) {
...
export default function App() {
  ...
  // Detected states (with time-based debounce)
  const [stableState, setStableState] = useState<FocusState>('focused');
  const pendingStateRef = useRef<FocusState>('focused');
  ...
  // full-screen reminder stays until the break countdown completes.
  const [breakMode, setBreakMode] = useState(false);
  const [breakSecs, setBreakSecs] = useState(0);
  const breakProgressRef = useRef<number>(0);

  // BREAK-GUARD ANCHOR — persisted to localStorage so reloads / redeploys
  // never lose interval progress. The current interval is measured as the
  // DELTA between today's stats and this anchor:
  //   focusInInterval = totalFocusTime  - anchor.focus
  //   restInInterval  = (distracted+absent) - anchor.rest
  const GUARD_KEY = 'kapi_guard_anchor';
  const guardAnchorRef = useRef<{ date: string; focus: number; rest: number }>({
    date: getTodayDateStr(),
    focus: 0,
    rest: 0,
  });
  const todayStatsRef = useRef<DailyStats>(emptyStats(getTodayDateStr()));
  useEffect(() => { todayStatsRef.current = todayStats; }, [todayStats]);

  const resetGuardAnchor = (stats: DailyStats) => {
    const a = {
      date: stats.date,
      focus: stats.totalFocusTime,
      rest: stats.totalDistractedTime + stats.totalAbsentTime,
    };
    guardAnchorRef.current = a;
    try { localStorage.setItem(GUARD_KEY, JSON.stringify(a)); } catch (_) {}
  };

  // Timers only run while the Focus Lens is actually sensing (camera+model or simulator)
  ...
  useEffect(() => {
    ...
    if (cachedSettings) {
      try {
        ...
        // MIGRATION: any settings saved before v19 get the new default model
        // URL and the new sound defaults (voice + chimes OFF until enabled).
        if (!parsed.settingsVersion || parsed.settingsVersion < SETTINGS_VERSION) {
          parsed.modelUrl = DEFAULT_SETTINGS.modelUrl;
          parsed.speechEnabled = false;
          parsed.chimeEnabled = false;
          parsed.userName = parsed.userName ?? '';
          parsed.settingsVersion = SETTINGS_VERSION;
          localStorage.setItem('kapi_settings', JSON.stringify(parsed));
        }
    ...
    let loadedStats: DailyStats = emptyStats(todayDate);
    if (cachedStats) {
      try {
        const parsed = JSON.parse(cachedStats);
        // MIGRATION: old stats format used phone/away/tired fields — merge them
        const migrated: DailyStats = {
          ...emptyStats(todayDate),
          ...parsed,
          totalDistractedTime:
            parsed.totalDistractedTime ??
            (parsed.totalPhoneTime || 0) + (parsed.totalTiredTime || 0),
          totalAbsentTime: parsed.totalAbsentTime ?? (parsed.totalAwayTime || 0),
          totalStretchTime: parsed.totalStretchTime || 0,
        };
        loadedStats = migrated;
        setTodayStats(migrated);
      } catch (e) {
    ...
    // Restore the break-guard anchor — page reloads must NOT restart the
    // focus interval. Only start fresh if there is no valid anchor for today.
    try {
      const rawAnchor = localStorage.getItem(GUARD_KEY);
      const a = rawAnchor ? JSON.parse(rawAnchor) : null;
      if (
        a &&
        a.date === todayDate &&
        typeof a.focus === 'number' &&
        typeof a.rest === 'number' &&
        a.focus <= loadedStats.totalFocusTime &&
        a.rest <= loadedStats.totalDistractedTime + loadedStats.totalAbsentTime
      ) {
        guardAnchorRef.current = a;
      } else {
        resetGuardAnchor(loadedStats);
      }
    } catch (_) {
      resetGuardAnchor(loadedStats);
    }

    const freshStreak = calculateWeeklyAndStreak();

    setTimeout(() => {
      triggerKapiSpeech('greet', { streak: freshStreak });
    }, 1200);
  ...
  // ---------------------------------------------------------
  // MIDNIGHT ROLLOVER WATCHER (idle/paused): if the page stays open past
  // 00:00 without the study loop ticking, this still flips "today" over to
  // the new date. Checks every 30s and whenever the tab becomes visible
  // again (covers laptops waking from sleep the next day).
  // ---------------------------------------------------------
  useEffect(() => {
    const rolloverIfNewDay = () => {
      const nowDate = getTodayDateStr();
      if (todayStatsRef.current.date !== nowDate) {
        const fresh = emptyStats(nowDate);
        todayStatsRef.current = fresh;
        setTodayStats(fresh);
        try { localStorage.setItem(`kapi_stats_${nowDate}`, JSON.stringify(fresh)); } catch (_) {}
        resetGuardAnchor(fresh);
        calculateWeeklyAndStreak();
      }
    };
    const id = setInterval(rolloverIfNewDay, 30_000);
    const onVis = () => { if (!document.hidden) rolloverIfNewDay(); };
  ...
  // Recalculate streak days and weekly records. Returns the streak so the
  // greeting can use the FRESH value (state updates are async — reading
  // streakDays right after calling this would still see the old value).
  const calculateWeeklyAndStreak = (): number => {
  ...
  // ---------------------------------------------------------
  useEffect(() => {
    // During an enforced break the WHOLE study loop is frozen — no session
    // seconds, no per-state time. Break time simply doesn't count anywhere.
    if (sessionState === 'running' && sensingActive && !breakMode) {
      timerRef.current = setInterval(() => {
        setSessionSeconds((prev) => prev + 1);
        sessionTickRef.current += 1;

        // MIDNIGHT ROLLOVER (in-session): if the clock has passed 00:00 while
        // studying, yesterday's stats stay saved under yesterday's key and a
        // FRESH DailyStats starts for the new date — daily totals, oranges and
        // the break-guard interval all reset.
        const nowDateStr = getTodayDateStr();
        const dayRolled = todayStats.date !== nowDateStr;
        const updated: DailyStats = dayRolled ? emptyStats(nowDateStr) : { ...todayStats };
        if (dayRolled) {
          resetGuardAnchor(updated);
          calculateWeeklyAndStreak();
        }
        ...
        if (stableState === 'focused') {
          ...
          if (updated.totalFocusTime > 0 && updated.totalFocusTime % orangeSecs === 0) {
            ...
            if (settings.chimeEnabled) playSoftChime('orange');
            triggerKapiSpeech('pomodoro');
          }
        } else if (stableState === 'distracted') {
          updated.totalDistractedTime += 1;
        } else if (stableState === 'absent') {
          updated.totalAbsentTime += 1;
        ...
        // --- Health guard: within each focus interval, cumulative NEW
        // distracted+absent time is tracked. Reaching N focused minutes with
        // rest ≤ threshold → reminder. Rest exceeding threshold → you already
        // rested enough, the interval restarts fresh (announced visibly).
        // Computed as deltas vs a PERSISTED anchor, so a page reload can
        // never restart or lose the interval. ---
        if (settings.healthEnabled !== false) {
          const focusLimit = Math.max(5, settings.healthFocusMinutes ?? 50) * 60;
          const anchor = guardAnchorRef.current;
          const focusInInterval = updated.totalFocusTime - anchor.focus;
          const restInInterval =
            updated.totalDistractedTime + updated.totalAbsentTime - anchor.rest;

          if (restInInterval > breakNeeded) {
            // MORE than the threshold of rest inside this interval — counts
            // as a real break, so no reminder; start a fresh interval.
            resetGuardAnchor(updated);
            setSpeechBubble(`You've rested over ${settings.healthBreakMinutes ?? 5} minutes — fresh focus interval starts now 🌱`);
          } else if (focusInInterval >= focusLimit) {
            // Focused minutes hit the limit while rest stayed within the
            // threshold → time for the break reminder.
            resetGuardAnchor(updated);
            breakProgressRef.current = 0;
            setBreakSecs(0);
            setBreakMode(true);
            // Break reminder ALWAYS sounds — health first, toggle doesn't apply here
            playSoftChime('breakStart');
        ...
        // Persist EVERY tick — the health guard depends on accurate persisted
        // stats surviving reloads. Snapshots are still added every 10 seconds.
  ...
  }, [sessionState, stableState, todayStats, settings, breakMode, sensingActive]);

  // ---------------------------------------------------------
  // ENFORCED BREAK COUNTDOWN — pure wall-clock, no camera needed.
  // The 5-minute rest bar fills every second no matter what you do:
  // step away, play the game, listen to music... it's YOUR break. 🍵
  // ---------------------------------------------------------
  useEffect(() => {
    if (!breakMode) return;
    const breakNeeded = Math.max(1, settings.healthBreakMinutes ?? 5) * 60;
    ...
    const interval = setInterval(() => {
      ...
      if (breakProgressRef.current >= breakNeeded) {
        ...
        breakProgressRef.current = 0;
        setBreakSecs(0);
        resetGuardAnchor(todayStatsRef.current);
        // Break-end also ALWAYS sounds — you might be away stretching 🍵
        playSoftChime('breakEnd');
  ...
  useEffect(() => {
    ...
    if (sessionState === 'running' && sensingActive) {
  ...
  }, [stableState, sessionState, todayStats, settings, sensingActive]);

  // ---------------------------------------------------------
  // SENSOR DETECTION & DEBOUNCE (state must persist ~5s to switch)
  // ---------------------------------------------------------
  const applyStateSwitch = (detectedState: FocusState) => {
    setStableState(detectedState);
    const nowMs = Date.now();

    if (detectedState === 'distracted') {
      const updatedStats = {
        ...todayStats,
        distractionCount: todayStats.distractionCount + 1,
        snapshots: [
          ...(todayStats.snapshots || []),
          { timestamp: nowMs, state: 'distracted' as FocusState },
        ],
      };
      saveStats(updatedStats);
      // no chime here — switching to distracted stays silent
      setSpeechBubble(`Looks like something caught your eye~ Let's refocus together! 🧡`);
    } else if (detectedState === 'absent') {
      setSpeechBubble(`${settings.mascotName} will nap right here and wait for you zZZ`);
    } else if (detectedState === 'stretch') {
      setSpeechBubble(`Great job!! ${settings.mascotName} is proud of you! 💮`);
    } else if (detectedState === 'focused') {
      setSpeechBubble(`Welcome back! Let's keep going~ 🚀`);
    }
  };

  // Time-based debounce: a new state must be detected CONTINUOUSLY for the
  // required duration before the timer actually switches — this avoids the
  // quick focused ↔ distracted jumping from single noisy frames.
  const SWITCH_DELAY_MS: Record<FocusState, number> = {
    distracted: 3000, // ≥3s of sustained detection before leaving focused
    absent: 3000,
    focused: 1500,
    stretch: 1500,
  };

  const handleSensorDetected = (detectedState: FocusState, immediate: boolean = false) => {
    if (immediate) {
      // Simulator buttons switch instantly for demo/testing
      pendingSinceRef.current = 0;
      if (detectedState !== stableState) applyStateSwitch(detectedState);
      return;
    }

    if (detectedState === stableState) {
      // back to the current stable state — cancel any pending switch
      pendingSinceRef.current = 0;
      return;
    }

    const now = Date.now();
    if (pendingStateRef.current !== detectedState || pendingSinceRef.current === 0) {
      // a NEW candidate state — restart its clock
      pendingStateRef.current = detectedState;
      pendingSinceRef.current = now;
      return;
    }

    if (now - pendingSinceRef.current >= SWITCH_DELAY_MS[detectedState]) {
      pendingSinceRef.current = 0;
      applyStateSwitch(detectedState);
    }
  };
  ...
  // ---------------------------------------------------------
  const startSession = () => {
    if (settings.chimeEnabled) playSoftChime('sessionStart');
  ...
  const generateDiagnosticSummary = async () => {
    ...
    try {
      ...
      saveStats({
        ...todayStats,
        aiSummary: `【${settings.mascotName}'s Study Summary】\nToday you spent a wonderful ${elapsedFocusMins} minutes in deep focus and harvested ${todayStats.pomodorosCompleted} oranges! ${todayStats.distractionCount} little distraction${todayStats.distractionCount === 1 ? '' : 's'} didn't stop your progress. Rest well — I'll be waiting at the study buddy corner tomorrow! 🍵`,
      });
    } finally {
  ...
  const stateChips: Array<{ key: FocusState; icon: React.ComponentType<any>; label: string; secs: number }> = [
    { key: 'focused', icon: BookOpen, label: 'Focused', secs: todayStats.totalFocusTime },
    { key: 'distracted', icon: Smartphone, label: 'Distracted', secs: todayStats.totalDistractedTime },
    { key: 'absent', icon: DoorOpen, label: 'Absent', secs: todayStats.totalAbsentTime },
  ];
  ...
  return (
    <div className="min-h-screen text-[#725442] flex flex-col font-sans relative">
        ...
        <div className="max-w-3xl mx-auto px-5 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-sm font-extrabold tracking-tight">
              {(settings.userName && settings.userName.trim()) ? settings.userName.trim() : settings.mascotName}'s Study Buddy
            </h1>
          </div>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-5 py-6">
        {/* PAGE 1: STUDY — always MOUNTED (hidden with CSS on other pages) so
            the webcam stream and the Focus Lens keep running while you check
            the Report or Settings pages. */}
          <div
            className={
              currentPage === 'study'
                ? 'grid grid-cols-1 md:grid-cols-12 gap-5 items-start'
                : 'hidden'
            }
          >
                ...
                <div className="mt-5 text-center">
                  <div className="text-[9px] uppercase tracking-[0.22em] text-[#B99C86] font-bold">
                    {sessionState === 'running'
                      ? (sensingActive ? 'studying' : 'lens off — timing paused')
                      : sessionState === 'paused' ? 'paused' : 'session'}
                  </div>
                  <div className={`text-5xl font-black font-mono tracking-tight leading-none mt-1 transition-colors ${
                    sessionState === 'running' && sensingActive ? 'text-[#FF8D66]' : 'text-[#725442]'
                  }`}>
                    {formatHMS(sessionSeconds)}
                  </div>
                  {sessionState === 'running' && !sensingActive && (
                    <p className="text-[10px] font-bold text-[#E8946A] mt-1.5 leading-snug">
                      Turn on the Focus Lens (camera) →<br />time starts counting when Kapi can see you 🍊
                    </p>
                  )}
                </div>
              ...
              <div className="order-3 md:col-span-3">
                <WebcamClassifier
                  ...
                  onSensingChange={setSensingActive}
                />
              </div>
          </div>
      ...
      <AnimatePresence>
        {breakMode && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] overflow-y-auto p-5"
          ...
          >
            ...
            <motion.div
              ...
              className="w-full max-w-lg text-center py-4"
            >
              <motion.h2
              ...
              <div className="mt-4 max-w-xs mx-auto pointer-events-none">
                <CompanionMascot
              ...
              <div className="mt-5 max-w-sm mx-auto">
                ...
                <p className="text-[10px] font-bold text-[#A0836D] mt-1.5">
                  🌱 your break is counting down — step away, stretch, play or just relax
                </p>
              </div>

              {/* Actions — game / music / skip */}
              <div className="mt-4 flex items-center justify-center gap-2.5 flex-wrap">
                ...
                <button
                  onClick={() => window.dispatchEvent(new CustomEvent('kapi-open-sound-panel'))}
                  className="px-5 py-2.5 bg-[#8EAC50] hover:bg-[#7D9B40] text-white text-xs font-bold rounded-xl flex items-center gap-1.5 active:scale-95 transition-all cursor-pointer shadow-md"
                >
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-stone-200 hover:bg-stone-300 text-stone-500 text-xs font-bold rounded-xl active:scale-95 transition-all cursor-pointer"
                >
                  Skip
                </button>
              </div>
          ...
          >
            ...
            >
              ...
              <div className="mt-4 space-y-4 text-xs text-[#725442] leading-relaxed">
                ...
                <div>
                  <h3 className="font-extrabold mb-1">2 · Three states it understands</h3>
                  <p className="text-[#A0836D]">
                ...
                <div>
                  ...
                  <p className="text-[#A0836D]">
                    Train an <b>Image Project</b> on Teachable Machine with the three class names above
                    (Focus / Distracted / Absent),
